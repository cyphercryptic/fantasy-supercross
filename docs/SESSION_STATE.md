...
### New this session (2026-07-01) — recap highlight bug + system audit (all live)
...
- **FIXED: activity log showed stale SX classes (250E/250W) in the MX league.** The transactions rider lookup in `free-agents` route read `riders.class`/`riders.number` raw. Now overridden from `rider_series` (same block as roster). Covers both the team page and free-agents page (both consume this endpoint). 11 of 27 txn riders were affected. Commit `4ebc5aa`. See memory [[stale-sx-class-override]].
- **FIXED: schedule race-results mis-bucketed MX riders.** `/api/races?id=` returned raw `riders.class/number/team`, so the schedule page split MX results into wrong 450/250 lists. Overridden from `rider_series` for non-SX races. Commit `c8597f3`.
- **FIXED: `/api/news` rider_injuries query was unbounded** (row-cap risk, same shape as the R4 card bug). Added `.limit(500)` newest-first (54 rows today). Commit `c8597f3`.
- **SYSTEM AUDIT (3 parallel sweeps + prod data checks) — clean except one data gap.** Verified: all scoring paths use `weekly_lineups` (no roster-vs-lineup misuse left); lineup writes gated by `isRaceLocked`, drops only touch `upcoming` races, add/drops blocked while a race is active (incl. MX mid-motos which stay `upcoming`); all `race_results`/`bonuses`/`lineups` reads properly scoped (row-cap safe); prod data solid — all 4 completed rounds have exactly 16 lineup riders/user + all 4 holeshots, rosters 20/20, no duplicate rider names.
- **OPEN DATA GAP (see open items #6):** 6 fringe 250 privateers raced MX but have no `rider_series` mx row (importer name-matched them to existing SX rider records so the Unmatched alert never fired). Rider ids **54 (Devin Simonson), 56 (Jeremy Hand), 64 (Hayes Edwards), 67 (Bryton Carroll), 70 (John Short IV), 77 (Jace Kessler)**. All scored 0-3 pts, none rosterable → standings unaffected, but they're missing from free agents + show stale SX class. Backfill deferred pending their real MX numbers (stale SX numbers may not match the entry list).
...
6. **PENDING (2026-07-01 audit): backfill 6 riders missing `rider_series` mx rows.** Rider ids **54 Devin Simonson, 56 Jeremy Hand, 64 Hayes Edwards, 67 Bryton Carroll, 70 John Short IV, 77 Jace Kessler** raced R3/R4 MX (0-3 pts each, none rostered → standings unaffected) but the importer name-matched them to existing SX rider rows, so they have no mx `rider_series` overlay — missing from MX free agents + showing stale SX class. Add mx `rider_series` rows with their **real MX numbers** (verify vs the promotocross entry list; stale SX numbers 89/87/146/483/90/511 may be wrong). Low priority — fringe privateers.
